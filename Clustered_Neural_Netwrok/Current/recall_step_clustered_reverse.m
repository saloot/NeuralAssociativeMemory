...
function [x_out] = recall_step_clustered_reverse(W,x,N,gamma_BFO,gamma_BFS,y_min,y_max,recall_algorithm_option,fixed_set)
...
while (exit_flag == 0)
    ...
    for iii = 1:N_const                
        if (abs(c_temp_WTA(iii))<.015)                        % Take care of numerical issues                        
            c_temp_WTA(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFO(iii))<.015)                        % Take care of numerical issues                        
            c_temp_BFO(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFS(iii))<.015)                        % Take care of numerical issues                        
            c_temp_BFS(iii) = 0;                                
        end        
    end
    ...
    if ( ((norm(c_WTA) < 0.0001)&&(norm(c_BFO) < 0.0001)&&(norm(c_BFS) < 0.0001) )||(itr > 40))                                                            
        exit_flag = 1;                                                  
        break;
    end    
    %----------------------------------------------------------------------                                                                       
    ...
    x_WTA(inde)=x_WTA(inde)-sign(x_temp_WTA(inde))*fixed_set(inde);             % Update the code nodes.                                        
    %----------------------------------------------------------------------
    ...
    for jjj = 2:N+1                                
        if ( ( x_temp2(jjj)>= gamma_BFO) && (sum(abs(sign(W(:,jjj))))>=2))          % Update all the nodes that receive a lot of feedback from their neighbors.                                                                              
    ...
    x_BFS = x_BFS - tempp.*fixed_set;           
    %----------------------------------------------------------------------
        
        
    x_WTA = max(x_WTA,y_min*ones(1,N+1));         % Wacth for saturations            
    x_WTA = min(x_WTA,y_max*ones(1,N+1));         % Wacth for saturations                    
    x_BFO = max(x_BFO,y_min*ones(1,N+1));         % Wacth for saturations            
    x_BFO = min(x_BFO,y_max*ones(1,N+1));         % Wacth for saturations                    
    x_BFS = max(x_BFS,y_min*ones(1,N+1));         % Wacth for saturations            
    x_BFS = min(x_BFS,y_max*ones(1,N+1));         % Wacth for saturations                    
