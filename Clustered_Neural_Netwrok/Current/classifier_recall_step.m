...
function [x_out,itr_total] = classifier_faultyrecall_step(W,x,N,gamma_BFO,gamma_BFS,y_min,y_max,recall_algorithm_option,pattern_neur_noise,const_neur_noise,const_update_threshold)
...
while (exit_flag == 0)
    ...
    x_temp2 = u + abs(c_BFS'*W)./sum(abs(W));               % x_temp2 is the raw feedback received by each pattern node.                                                       
