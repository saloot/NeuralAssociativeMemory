...
function clustered_neural_learn(no_clusters,cluster_size,cluster_index,const_learn,alpha0,beta0,theta0,db_file_in,db_name_in,learn_itr_max,Q,sigma1,mean1,simulation_set)
...
while ( (learned_consts <= const_learn)  && (total_itr < total_itr_max) )
    ...
    W = random_vector(cluster_size+1,2*round(log(cluster_size)));
    % W = W';
%     W = rand(1,cluster_size+1);
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        for ii = 1:dataset_size
            ...
            x = [8,x_temp(1+index_pattern_neurons)];                                    
            %--------------------------------------------------------------
            ...
            if (norm(W) > .001)                                
%                 W = W' - alph*y*( x' - (y*W'/(norm(W))^2))-beta0*soft_threshold_inv(W,0.015);%W.*(1-(tanh(100*W(j,:).^2)).^2)
%                 W = W';
...
%                     W(ij,:) = soft_threshold(W(ij,:) - alph*y(ij)*(x-y(ij)*W(ij,:))+ alph*(1-norm(W(ij,:))^2)*W(ij,:),theta);                           
                if (learn_itr > 2)
                    if (abs(y)/norm(x(2:end)) < .1)
                        W = soft_threshold(W - alph*y*(x-y*W)+ 2*alph*(1-norm(W)^2)*W,theta);
                        W = W';
                    else
                        index_exceptionl = [index_exceptionl,mu];
                    end
                else
                    W = soft_threshold(W - alph*y*(x-y*W)+ 2*alph*(1-norm(W)^2)*W,theta);                           
                    W = W';
...
%                 end
                % W = soft_threshold_matrix(W - alph*y*( x - (y'*(W./(sqrt(sum(W'.*W'))'*ones(1,cluster_size+1))))),theta0);%-beta0*W.*(1-(tanh(100*W.^2)).^2);                    
            end            
            %--------------------------------------------------------------
        ...
        converged_count = sum(abs(W2*dataset_learn')<max_y_threshold);
        cost_one_itr = norm(W2*dataset_learn')^2-norm(W2*dataset_learn(index_exceptionl,:)')^2;
...
%         end
        ...
        if (learn_itr > 10)
            if (norm(mean(cost_per_itr_av(end-9:end-1))-cost_one_itr)/mean(cost_per_itr_av(end-9:end-1))<.0005)
%             if (norm(mean(no_converged_patts_per_itr(end-9:end-1))-converged_count)/mean(no_converged_patts_per_itr(end-9:end-1))<.001)
                break;
...
%     for ii = 1:mi       
%         for jj = ii+1:mi
%             if ((norm(sign(W_temp(:,ii))-sign(W_temp(:,jj)))<.1) || (norm(sign(W_temp(:,ii))+sign(W_temp(:,jj)))<.1))
%                 if (norm(sign(W_temp(:,ii)))>0) 
% %                     success_flag = 0;                 
%                     break;             
%                 end        
%             end
