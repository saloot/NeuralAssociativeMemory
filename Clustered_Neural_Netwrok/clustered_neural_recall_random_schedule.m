...
%     end
    ...
    counter = 0;
    [total_no_consts,~] = size(W_global_tot);
    x = [1,x];
    while ((try_itr < try_max) && (success_flag == 0))
        ...
        temp_ind = randperm(total_no_consts);
        temp_ind = temp_ind(1:no_activated_consts);
        
        W_local = W_global_tot(temp_ind,:);
        W_local_deg_dist = sum(abs(sign(W_local)));
        if  (sum((abs(nois).*W_local_deg_dist(2:end))>2) > 0)
            111;
        end
            %--------------------------------------------------------------    
...
%             [x_out] = recall_step_clustered_real_value(W,x_temp,.05,y_min,y_max,0);    
        [x_out,update_term] = recall_step_clustered_v2(W_local,x,varphi,psi,y_min,y_max,Q);
        %--------------------------------------------------------------
...
%             if (norm(W*x_out')< norm(W*x_temp'))       % If there is an error due to the recall process (and not an unlearned pattern) then update error rates.                       
        if ( (norm(W_local*x_out') < norm(W_local*x')) )
