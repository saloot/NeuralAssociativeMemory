...
function clustered_neural_learn_v7(no_clusters,cluster_size,cluster_index,const_learn,alpha0,beta0,theta0,db_file_in,db_name_in,learn_itr_max,Q,sigma1,mean1,simulation_set,no_of_PC)
...
max_y_threshold = .001;                                                          % The maximum acceptable projection for a pattern on a weight vector
...
%     W = rand(1,cluster_size+1);
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        cost_one_itr = norm(W*dataset_learn_temp')^2;
        %------------------------------------------------------------------        
        ...
        if ( (converged_count>fraction_of_convergence*dataset_size) && (norm(W)>.1))
            W = soft_threshold(W,max_y_threshold+0.0015);
            W = W';
            converged_count = sum(abs(W*dataset_learn_temp')<max_y_threshold);
