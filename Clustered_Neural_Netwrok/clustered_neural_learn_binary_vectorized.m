...
% function clustered_neural_learn_binary_vectorized(no_clusters,cluster_size,cluster_index,const_learn,alpha0,beta0,theta0,db_file_in,db_name_in,learn_itr_max,Q,sigma1,mean1,simulation_set,no_of_PC)
...
max_y_threshold = .04;                                                          % The maximum acceptable projection for a pattern on a weight vector
min_W = .1;                                                                     % The minimum absolute value of the weight vectors
fraction_of_convergence = .78;                                                   % Required fraction of patterns to converge
...
% dataset_learn = vectorized_dataset;
...
temp = randperm(N);    
temp = 1:N;
temp2 = temp(2*cluster_size+1:3*cluster_size);
index_pattern_neurons = [];
for i = 1:cluster_size
    index_pattern_neurons = [index_pattern_neurons,temp2(i)*KK:(temp2(i)+1)*KK-1];
end
% index_pattern_neurons = temp(1:cluster_size*KK);
%%
%=============================LEARNING PHASE=============================== 
learned_consts = 0;
total_itr = 0;
...
while ( (learned_consts <= const_learn)  && (total_itr < total_itr_max) )
    ...
    W = random_vector(cluster_size*KK+1,round(1*(1+rand)*log(cluster_size*KK)));
    % W = W';
%     W = rand(1,cluster_size+1);
    W = W./(sqrt(sum(W'.*W'))'*ones(1,cluster_size*KK+1));    
    %----------------------------------------------------------------------
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        for ii = 1:dataset_size
            ...
            if (norm(W) > .001)                                                    
                W = soft_threshold(W - alph*y*(x-y*W)+ .75*alph*(1-norm(W)^2)*W,theta);                                               
                W = W';
