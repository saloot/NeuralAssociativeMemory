...
for net_simul_itr = 1:no_simulated_instances                % Simulate the error correction procedure for the given ensemble.                                
    ...
    while ((try_itr < try_max) && (success_flag == 0))
        ...
        for c = 1:no_of_constraints
            W_cluster = W(c,:);
            noise_effective = (x-pattern).*sign(abs(W_cluster));
            ...
            [x_out] = recall_step_clustered_scheduled(W_cluster,x,N,gamma_BFO,0,1);
            %--------------------------------------------------------------
            
            %----------Verify the Success of the Recall Algorithm----------                
            if ((norm(W_cluster*x_out') -norm(W_cluster*x')<-1e-1)&&(norm(W_cluster*pattern')<.1))       % If there is an error due to the recall process (and not an unlearned pattern) then update error rates.                                       
                x =x_out;            
                mean(W*x')
            ...
            if (norm(W*x')-norm(W*pattern')<-.1*norm(W*pattern'))
                success_flag = -1;
                break
            end
            %--------------------------------------------------------------
