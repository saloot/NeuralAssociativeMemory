...
for ll = 1:length(N_in)
    ...
    processed_BER = processed_BER/(N_tot/N);
    %----------------------------------------------------------------------                       
    ...
    [lambda_edge,rho_edge] = node_to_edge_deg_distribution(lambda,rho);
...
%     [PER_tehroy_worst2,PER_tehroy_ave2,BER_tehroy_worst2,BER_tehroy_ave2] = P_e_clustered_theory_main(N,K,L,lambda,rho,processed_error_bits,try_max,pe_max2,pe_ave2,pe_2_max,pe_2_ave);
    pe_ave2 = 1-(1-pe_ave2)^(N_in);
%     pe_max2 = 1-(1-pe_max2)^(N_in-1);
    [PER_tehroy_worst2,PER_tehroy_ave2,BER_tehroy_worst2,BER_tehroy_ave2] = P_e_clustered_theory_main(N,K,L,lambda_edge,rho_edge,processed_error_bits,try_max,pe_max2,pe_ave2,1,1);
    %----------------------------------------------------------------------
...
%     hold on
    ...
    hold on
    plot(sort(processed_error_bits),sort(processed_error_bits)/N_tot,'b--','LineWidth',2);
    hold on
    title('BER')    
end
%==========================================================================
