...
%     W = rand(1,cluster_size+1);
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        for ii = 1:dataset_size
            ...
            if (norm(W) > .001)                                                    
                W = soft_threshold(W - 1.25*alph*y*(x-y*W)+ .75*alph*(1-norm(W)^2)*W,theta);                                               
                W = W';
...
[mi,~] = size(W_global);  
if (mi > 1)
    converged_matrix = [converged_matrix;sum(abs(cost)<max_y_threshold)>=mi * 0.75];
end
