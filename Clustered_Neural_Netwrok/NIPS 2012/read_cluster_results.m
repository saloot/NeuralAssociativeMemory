...
for ll = 1:length(N_in)
    ...
    [~,deg_column_within,lambda_within,~] = read_cluster_degree_local(N,K,L,alpha0,beta0,theta0);
    %----------------------------------------------------------------------
    
    %---------------------------Compare with Theory------------------------
    [PER_tehroy_single] = P_e_clustered_theory_single(N,K,L,lambda,rho,processed_error_bits,try_max);

    [PER_tehroy_multi] = P_e_clustered_theory_multi(N,K,L,lambda,deg_column,rho,deg_row,processed_error_bits,1,gamma_BFS,try_max,lambda_within,deg_column_within);  
    %----------------------------------------------------------------------
    ...
    temp_lega = [temp_leg,' - Theory sing'];
    figure_legend = [figure_legend;temp_lega];
    temp_lega = [temp_leg,' - Theory mult'];
    figure_legend = [figure_legend;temp_lega];
...
%     figure(h_error_PER);
    plot(sort(processed_error_bits)/N_tot,sort(PER_tehroy_single),'b','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);
    hold on
    plot(sort(processed_error_bits)/N_tot,sort(PER_tehroy_multi),'b-.','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);
    hold on
    plot(sort(processed_error_bits)/N_tot,sort(processed_PER),'b--','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);
    hold on
