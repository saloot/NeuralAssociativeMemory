...
[dataset_size,N] = size(dataset_learn_orig);
%--------------------------------------------------------------------------
...
fraction_of_convergence = .95;                                                   % Required fraction of patterns to converge
...
%     dataset_learn = round(Q*tanh(sigma1*dataset_learn-mean1));
    mean1 = mean(mean(dataset_learn_orig));
    mean1 = 0;
    dataset_learn  = dataset_learn_orig-mean1;
    dataset_learn = round(Q*dataset_learn);
    dataset_learn_orig = dataset_learn;
...
%     
%     c = sign(dataset_learn-dataset_projected);
%     s = (sum(abs(c')));
%     [val,ind] = sort(s);
...
%     end
%     ind = ind(1:i-1);
%     dataset_projected = dataset_projected(ind,:);
%     
...
while ( (learned_consts <= const_learn)  && (total_itr < total_itr_max) )
    ...
    dataset_learn_temp = dataset_learn(:,index_pattern_neurons);
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        for ii = 1:dataset_size
            ...
            if (norm(W) > .001)                                                    
                W = soft_threshold(W - alph*y*(x-y*W)+ 1*alph*(1-norm(W)^2)*W,theta);
%                 W = soft_threshold(W - alph*y*(x)+ 2*alph*(1-norm(W)^2)*W,theta);                                               
                W = W';
            end            
            %--------------------------------------------------------------
        ...
        cost_one_itr = norm(W*dataset_learn_temp');
        %------------------------------------------------------------------        
        ...
        if ( (converged_count>fraction_of_convergence*dataset_size) && (norm(W)>.1))
            W = soft_threshold(W,2*max_y_threshold+0.001);
            W = W';
            converged_count = sum(abs(W*dataset_learn_temp')<max_y_threshold);
