...
function [x_out] = recall_step_clustered_noisy(W,x,gamma_BFO,gamma_BFS,y_min,y_max,recall_algorithm_option,max_y_threshold,pattern_neur_noise,const_neur_noise)
...
while (exit_flag == 0)
    ...
    c_temp_WTA = W*x_WTA' + const_neur_noise;                                      % Find which constraint nodes are violated.                                       
    c_temp_BFO = W*x_BFO' + const_neur_noise;                                      % Find which constraint nodes are violated.                                       
    c_temp_BFS = W*x_BFS' + const_neur_noise;                                      % Find which constraint nodes are violated.                                       
    ...
    x_temp2 = abs(c_BFO')*abs(sign(W))./sum(abs(sign(W))) + pattern_neur_noise;               % x_temp2 is the raw feedback received by each pattern node.                                                       
    ...
    x_temp2 = abs(c_BFS'*W)./sum(abs(W)) + pattern_neur_noise;               % x_temp2 is the raw feedback received by each pattern node.                                                       
