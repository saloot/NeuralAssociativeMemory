...
function clustered_neural_learn_v9(no_clusters,cluster_size,cluster_index,const_learn,alpha0,beta0,theta0,db_file_in,db_name_in,learn_itr_max,Q,simulation_set)
...
fraction_of_convergence = .55;                                                   % Required fraction of patterns to converge
...
% %     dataset_learn = dataset_learn/max(max(abs(dataset_learn)));
    dataset_learn = round(Q*tanh(dataset_learn))/Q;
...
while ( (learned_consts <= const_learn)  && (total_itr < total_itr_max) )
    ...
    W = random_vector(cluster_size+1,round(2*(1+rand)*log(cluster_size)));
    % W = W';
%     W = rand(1,cluster_size+1);
    ...
    for learn_itr = 1:learn_itr_max                         % Repeat the steps below several times in order to enhance the quality of the learning phase        
        ...
        if (mod(learn_itr,42) == 1)            
            alph_itr = alph_itr + 1;
            alph0 = max(14*alpha0/(13+alph_itr),0.0005);        
        end        
...
%                 W = W - alph*y*( x - (y*W/(norm(W))^2))-beta0*(soft_threshold_inv(W,theta))';
        ...
        if (mod(learn_itr,25) == 0)
            theta_itr = theta_itr + 1;
            theta = 3*theta0/(2+theta_itr);              
        end
        
        if (mod(learn_itr,150) == 0)
            W = soft_threshold(W,2*max_y_threshold+.0015);
            W = W';
        end
        %--------------------------------------------------------------
        ...
        if ( (converged_count>fraction_of_convergence*dataset_size) && (norm(W)>.1))
            W = soft_threshold(W,2*max_y_threshold+0.0015);
            W = W';
            converged_count = sum(abs(W*dataset_learn_temp')<max_y_threshold)
   ...
   if (success_flag)
       ...
       save([destination_folder_save,'/Simulation_Results_cluster_size_',num2str(cluster_size),'_consts_',num2str(const_learn),'_alpha_',num2str(alpha0),'_beta_',num2str(beta0),'_theta_',...
           ...
           'W_global','W_total','index_pattern_neurons','cost_per_itr_av','Learn_itr_cell','max_y_threshold');
       %--------------------------------------------------------------------------
   end
   [mi,~] = size(W_global);
