...
function [Perr, Pvc, Pcv] = DE_irregLDPC_BSC(p,dv,dc,del,N,max_itr)

converge_val = 1e-8;    % We declare convergence when the values of Perr 
                        % over two successive iterations do not differ by 
...
%     Perr(itr)
    ...
    if ( (itr>max_itr))
        end_flag = 1;
    elseif  (itr >=2)
        if (abs(Perr(itr)-Perr(itr-1)) < converge_val)
            end_flag = 1;
        end
    end
end
...
Pvc = roundoff(Pvc);    % Check to see if it is a valid PDF (sums to 1), compensate carefully for numerical issues
Pcv = roundoff(Pcv);
...
c = max(c,0);  % Fix numerical issues that result in small negative values

%-----------------------Check the Validity of the Output-------------------
if (sum(c)~=1)    
    %----------------------Check for Numerical Issues----------------------
    if (abs(sum(c)-1)>1e-6)
        error('Error in the convoluted density of the variable update function');
...
function P = roundoff(c)
% Check to see if it is a valid PDF (sums to 1), compensate carefully for
% numerical issues
c = max(c,0);
M = length(c);
%-----------------------Check the Validity of the Output-------------------
