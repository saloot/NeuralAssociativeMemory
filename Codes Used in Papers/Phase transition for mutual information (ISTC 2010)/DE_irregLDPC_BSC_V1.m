...
while (end_flag == 0),
    ...
    for i = 1:length(dc(1,:))
        ...
        Pcv = conv_chk(Pvc_final,Pvc_final,del,T);        % Need dc >= 3
        if (dc(1,i)>=3)
            for cnt1 = 1:dc(1,i)-3,
                Pcv = conv_chk(Pcv,Pvc_final,del,T);
            end
        end
    ...
    for i = 1:length(dv(1,:))
        Pvc = conv_var(P0,Pcv_final);        % Need dv >= 3
        if (dv(1,i)>=2)
            for cnt1 = 1:dv(1,i)-2,
                Pvc = conv_var(Pvc,Pcv_final);
            end
        end
    ...
    Perr(itr) = sum(Pvc_final(1:N)) + 0.5*Pvc_final(N+1);
    Perr(itr)
    %-------------Determine Check to Variable Node Messages----------------
    ...
    if ( (itr>max_itr))
        end_flag = 1;
    elseif  (itr >=2)
        if (abs(Perr(itr)-Perr(itr-1))/Perr(itr) <1e-2 )
            end_flag = 1;
        end
    end
end
