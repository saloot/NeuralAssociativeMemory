...
%         h_error_PER_WTA = figure;    
    ...
    hold on
    for ii = 1:length(no_of_patterns_range)        
        plot(sort(final_processed_error_bits_Jankowski_non_random(ii,:)),sort(final_processed_BER_Jankowski_non_random(ii,:)),'b-.','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);    
        plot(sort(final_processed_error_bits_Jankowski_random(ii,:)),sort(final_processed_BER_Jankowski_random(ii,:)),'b-*','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);    
    end
