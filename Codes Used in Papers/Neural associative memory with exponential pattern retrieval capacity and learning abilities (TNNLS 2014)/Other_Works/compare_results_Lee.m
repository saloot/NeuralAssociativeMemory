...
no_of_patterns_range = [400,500,800,1000,2000];
% function [processed_error_bits_BFO,processed_PER_WTA,processed_PER_BFO,processed_PER_BFS] = read_Lee_results(N_in,K_in,alpha0,beta0,theta0,gamma_BFO,gamma_BFS)
warning off
%%
%==============================INITIALIZATION==============================
...
for ll = 1:length(N_in)
    ...
    final_processed_error_bits_Lee_random = [];
    final_processed_error_bits_Lee_non_random = [];
    
    final_processed_PER_Lee_random = [];
    final_processed_PER_Lee_non_random = [];
    
    final_processed_BER_Lee_random = [];
    final_processed_BER_Lee_non_random = [];
    
    final_processed_count_Lee_random = [];
    final_processed_count_Lee_non_random = [];
    ...
    for random_flag = 0:1
        for ii = 1:length(no_of_patterns_range)
            ...
            counter = counter + 1;
            processed_error_bits_Lee_random = [];
            processed_error_bits_Lee_non_random = [];
    
            processed_PER_Lee_random = [];
            processed_PER_Lee_non_random = [];
    
            processed_BER_Lee_random = [];
            processed_BER_Lee_non_random = [];
    
            processed_count_Lee_random = [];
            processed_count_Lee_non_random = [];
            ...
            if (fid > -1)
                results_Lee = fscanf(fid, '%s %d %s %f %s %f',[10,inf]);
                fclose(fid);                 
            else
                error('Undefined Lee input file!')
            end            
            unprocessed_error_bits_Lee = results_Lee(2,:);
            unprocessed_PER_Lee = results_Lee(6,:);
            unprocessed_BER_Lee = results_Lee(10,:);                
                        
            
            for i = 1:length(unprocessed_error_bits_Lee)
                if (random_flag)
                    processed_error_bits_Lee = processed_error_bits_Lee_random;
                else
                    processed_error_bits_Lee = processed_error_bits_Lee_non_random;
                end
                processed_flag = 0;
                for j = 1:length(processed_error_bits_Lee)
                    if (unprocessed_error_bits_Lee(i) == processed_error_bits_Lee(j))
                        processed_flag = 1;
                        break;
                    end
                end
                ...
                if (processed_flag == 0)
                    if (random_flag)
                        processed_error_bits_Lee_random = [processed_error_bits_Lee_random,unprocessed_error_bits_Lee(i)];
                        processed_BER_Lee_random = [processed_BER_Lee_random,unprocessed_BER_Lee(i)];
                        processed_PER_Lee_random = [processed_PER_Lee_random,unprocessed_PER_Lee(i)];
                        processed_count_Lee_random = [processed_count_Lee_random,1];
                    else
                        processed_error_bits_Lee_non_random = [processed_error_bits_Lee_non_random,unprocessed_error_bits_Lee(i)];
                        processed_BER_Lee_non_random = [processed_BER_Lee_non_random,unprocessed_BER_Lee(i)];
                        processed_PER_Lee_non_random = [processed_PER_Lee_non_random,unprocessed_PER_Lee(i)];
                        processed_count_Lee_non_random = [processed_count_Lee_non_random,1];
                    end
                else
                    if (random_flag)                    
                        processed_BER_Lee_random(j) = processed_BER_Lee_random(j) + unprocessed_BER_Lee(i);
                        processed_PER_Lee_random(j) = processed_PER_Lee_random(j) + unprocessed_PER_Lee(i);
                        processed_count_Lee_random(j) = processed_count_Lee_random(j) + 1;
                    else                    
                        processed_BER_Lee_non_random(j) = processed_BER_Lee_non_random(j) + unprocessed_BER_Lee(i);
                        processed_PER_Lee_non_random(j) = processed_PER_Lee_non_random(j) + unprocessed_PER_Lee(i);
                        processed_count_Lee_non_random(j) = processed_count_Lee_non_random(j) + 1;
                    end
        
                end
            end
            
            processed_BER_Lee_random = processed_BER_Lee_random./processed_count_Lee_random;                        
            processed_PER_Lee_random = processed_PER_Lee_random./processed_count_Lee_random;
            
            processed_BER_Lee_non_random = processed_BER_Lee_non_random./processed_count_Lee_non_random;                        
            processed_PER_Lee_non_random = processed_PER_Lee_non_random./processed_count_Lee_non_random;
            
            final_processed_error_bits_Lee_random = [final_processed_error_bits_Lee_random;processed_error_bits_Lee_random];
            final_processed_error_bits_Lee_non_random = [final_processed_error_bits_Lee_non_random;processed_error_bits_Lee_non_random];
    
            final_processed_PER_Lee_random = [final_processed_PER_Lee_random;processed_PER_Lee_random];
            final_processed_PER_Lee_non_random = [final_processed_PER_Lee_non_random;processed_PER_Lee_non_random];
    
            final_processed_BER_Lee_random = [final_processed_BER_Lee_random;processed_BER_Lee_random];
            final_processed_BER_Lee_non_random = [final_processed_BER_Lee_non_random;processed_BER_Lee_non_random];
    
            final_processed_count_Lee_random = [final_processed_count_Lee_random;processed_count_Lee_random];
            final_processed_count_Lee_non_random = [final_processed_count_Lee_non_random;processed_count_Lee_non_random];
...
%         h_error_PER_WTA = figure;    
    ...
    hold on
    for ii = 1:length(no_of_patterns_range)        
        plot(sort(final_processed_error_bits_Lee_non_random(ii,:)),sort(final_processed_PER_Lee_non_random(ii,:)),'b-.','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);    
        plot(sort(final_processed_error_bits_Lee_random(ii,:)),sort(final_processed_PER_Lee_random(ii,:)),'b-*','LineWidth',2,'Color',[tanh(ll/length(N_in)),ll/length(N_in),1-(ll/length(N_in))]);    
    end
