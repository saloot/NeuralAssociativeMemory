...
% x_max: The minimum value that a pattern node can have
% x_max: The minimum value that a pattern node can have
% -------------------------------------------------------------------------
...
function [x_WTA,x_BFO,x_BFS] = recall_step(W,x,N,N_const,gamma_BFO,gamma_BFS,max_noise_amp,err_bits,x_min,x_max,nu)
...
while (exit_flag == 0)
    ...
    for iii = 1:N_const                
        if (abs(c_temp_WTA(iii))<nu)                        % Take care of numerical issues                        
            c_temp_WTA(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFO(iii))<nu)                        % Take care of numerical issues                        
            c_temp_BFO(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFS(iii))<nu)                        % Take care of numerical issues                        
            c_temp_BFS(iii) = 0;                                
        end        
    end
    ...
    x_WTA = max(x_WTA,x_min*ones(1,N));         % Wacth for saturations            
    x_WTA = min(x_WTA,x_max*ones(1,N));         % Wacth for saturations                    
    x_BFO = max(x_BFO,x_min*ones(1,N));         % Wacth for saturations            
    x_BFO = min(x_BFO,x_max*ones(1,N));         % Wacth for saturations                    
    x_BFS = max(x_BFS,x_min*ones(1,N));         % Wacth for saturations            
    x_BFS = min(x_BFS,x_max*ones(1,N));         % Wacth for saturations                    
    %----------------------------------------------------------------------
