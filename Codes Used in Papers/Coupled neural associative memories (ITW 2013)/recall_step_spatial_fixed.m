%==========================================================================
%*****************FUNCTION: recall_step_spatial_fixed**********************
%==========================================================================

%--------------------------------INPUTS------------------------------------
% W: The connectivity matrix of the neural graph
% x: The corrupted pattern
% fixed_set: The index of the pattern nodes that are fixed. A zero in position i indicate that the pattern neuron i should be fixed to its original value.
...
% of our proposed algorithm in the Alerton 2012 paper, for the constrained 
% coupled neural systems. Here, constrained refers to the fact that the
% boundaries of the system are fixed. These boundaries are given by the
% vector fixed_set.

% The algorithm iteratively corrects the errors until either there is no
...
function [x_out] = recall_step_spatial_fixed(W,x,fixed_set,N,gamma_BFO,gamma_BFS,max_noise_amp,y_min,y_max,recall_algorithm_option)
...
while (exit_flag == 0)
    ...
    for iii = 1:N_const                
        if (abs(c_temp_WTA(iii))<.04)                        % Take care of numerical issues                        
            c_temp_WTA(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFO(iii))<.04)                        % Take care of numerical issues                        
            c_temp_BFO(iii) = 0;                                
        end        
        
        if (abs(c_temp_BFS(iii))<.04)                        % Take care of numerical issues                        
            c_temp_BFS(iii) = 0;                                
        end        
    end
    ...
    if ( ((norm(c_WTA) < 0.001)&&(norm(c_BFO) < 0.001)&&(norm(c_BFS) < 0.001) )||(itr > 20*max_noise_amp))                                                            
        exit_flag = 1; 
        break;
    end    
    %----------------------------------------------------------------------                                  
    ...
    tempp = zeros(1,N);                                        
    for jjj = 1:N                                    
        if (( x_temp2(jjj)> gamma_BFS)&&(deg(jjj)>4))          % Update all the nodes that receive a lot of feedback from their neighbors.                                                                              
