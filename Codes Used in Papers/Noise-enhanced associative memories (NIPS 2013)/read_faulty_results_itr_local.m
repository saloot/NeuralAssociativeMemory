...
function [processed_error_bits,processed_recall_itr_local,fraction_unsuccess_count_local] = read_faulty_results_itr_local(N_in,K_in,L_in,alpha0,beta0,theta0,gamma_BFO,gamma_BFS,recall_algorithm_option,pattern_neur_noise_range,const_neur_noise_range)
warning off
...
%         h_error_PER = figure;
        ...
        for ihi = 1:length(pattern_neur_noise_range)
            ...
            unprocessed_recall_itr_local = results_itr(6,:);
            ...
            for i = 1:length(unprocessed_error_bits)
                ...
                if (processed_flag == 0)
                    ...
                    processed_recall_itr_local = [processed_recall_itr_local,unprocessed_recall_itr_local(i)];                                            
                    processed_count = [processed_count,1];
                    
                    if (unprocessed_recall_itr_local(i)>=10000)
                        processed_unsuccess_count =[processed_unsuccess_count,1];
                    else
                        processed_unsuccess_count =[processed_unsuccess_count,0];
                    end
                else                                        
                    processed_recall_itr_local(j) = processed_recall_itr_local(j) + unprocessed_recall_itr_local(i);
            ...
            processed_recall_itr_local = processed_recall_itr_local - 10000*processed_unsuccess_count;            
            ...
            plot(sort(processed_error_bits)/N_tot,(sort(processed_recall_itr_local)),'b-','LineWidth',6,'Color',[tanh(ihi/length(pattern_neur_noise_range)),ihi/length(pattern_neur_noise_range),1-(ihi/length(pattern_neur_noise_range))]);
            hold on
...
% figure(h_error_PER)
% legend(figure_legend)
fraction_unsuccess_count_local = processed_unsuccess_count./processed_count;
