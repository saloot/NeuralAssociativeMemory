%==========================================================================
%*************FUNCTION: read_faulty_results_nu_upsilon*********************
%==========================================================================
...
% The final Bit Error Rates are shown in a graph as a function of upsilon,
% and nu, the internal noise parameters at pattern and constraint neurons,
% respectively.
%--------------------------------------------------------------------------
...
function [processed_pattern_noise,processed_recall_itr,fraction_unsuccess_count] = read_faulty_results_nu_upsilon(N_in,K_in,L_in,alpha0,beta0,theta0,gamma_BFO,gamma_BFS,recall_algorithm_option,pattern_neur_noise_range,const_neur_noise_range)
warning off
...
for ll = 1:length(N_in)
    ...
    for kji = 1:length(pattern_neur_noise_range)
        ...
        if (norm(sort(processed_pattern_noise)-pattern_neur_noise_range)<1e-6)
            BER_processed(:,kji) = sort_read_results(processed_pattern_noise,processed_BER);
            PER_processed(:,kji) = sort_read_results(processed_pattern_noise,processed_PER);
        else
