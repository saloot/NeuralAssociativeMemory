...
% cluster_index: The index of the cluster which we are interested in finding its degree distribution
%--------------------------------------------------------------------------
...
function [deg_row,deg_column,lambda,rho,d_max,d_min,d_ave,m,pe_max,pe_max2,pe_ave,pe_ave2,pe_2_max,pe_2_ave] = read_cluster_degree_local_v2(N,K,L,alpha0,beta0,theta0,cluster_index)
...
counter = 0;
%==========================================================================
saved_file = [pwd,'/Recall_Results/N_',num2str(N),'_K_',num2str(K),'_L_',num2str(L),'/Read_Results/sparsity_results_local_N_',num2str(N),...
    '_K_',num2str(K),'_L_',num2str(L),'_alpha0_',num2str(alpha0),'_beta_',num2str(beta0),'_theta_',num2str(theta0),'_cluster_index_',num2str(cluster_index),'.mat'];
...
for ind = 1:index_max       
    load(['/scratch/amir/Clustered_Neural/Initialization_Files/N_',num2str(N),'_K_',num2str(K),'_L_',num2str(L),...
        '/clustered_neural_parameters_v1_N_',num2str(N),'_K_',num2str(K),'_L_',num2str(L),'_index_',num2str(index),'_cluster_',num2str(cluster_index),'.mat']);        
    n = length(index_l);        
    fid = fopen(['/scratch/amir/Clustered_Neural/Learn_Results/N_',num2str(N),'_K_',num2str(K),'_L_',num2str(L),...
        '/Weigh_matrix_alpha_',num2str(alpha0),'_beta_',num2str(beta0),'_theta_',num2str(theta0),'_cluster_',num2str(cluster_index),'_index_',num2str(index),'.txt'], 'r');        
    if (fid >-1)                    
        ...
        m(ind) = a;
        N_const_tot(ind) = N_const_tot(ind) + m(ind);            
        fclose(fid);               
    end
end
%==========================================================================
...
for ind = 1:index_max                            
    ...
    if (fid >-1)                        
        ...
        d_min(ind) = mind;
        d_ave(ind) = sum(dd)/length(dd);

%             pe = lambda_poly(1-rho_poly(1-1/n,rho_temp/a),lambda_temp/n);
%             pe = lambda_poly(d_max(ind)/m(ind),lambda_temp/n);
        pe = ( d_ave(ind)/m(ind) )^d_min(ind);                   
        
        if (pe>pe_max2)
            pe_max2 = pe;            
        end
                
        temp_val = ( d_max(ind)/m(ind) )^d_min(ind);
%             temp_val = lambda_poly(d_max(ind)/m(ind),lambda_temp/n);
...
%             pe_1 = (1-(1-d_ave(ind)/m(ind))^2)^d_min(ind);
        pe_1 = (min(m(ind),2*d_max(ind))/m(ind))^d_min(ind);
...
%             end
        ...
        for i = 1:length(lambda_temp)                
            if (lambda_temp(i) > 0)                    
                ...
                pe_2 = 0;                
                for j = ceil(dp/2):dp                        
                    pe_2 = pe_2 + nchoosek(dp,j) * ((d_ave(ind)/m(ind))^j) * (1-(d_ave(ind)/m(ind)))^(dp-j);                    
                end                
...
%             pe_2 = 0;
%             for i = ceil(d_min(ind)/2):d_min(ind)
%                 pe_2 = pe_2 + nchoosek(d_min(ind),i) * ((d_ave(ind)/m(ind))^i) * (1-(d_ave(ind)/m(ind)))^(d_min(ind)-i);
%             end
%             
...
%=============================SAVE RESULTS=================================
if (~exist([pwd,'/Recall_Results/N_',num2str(N),'_K_',num2str(K),'_L_',num2str(L),'/Read_Results'],'dir'))
