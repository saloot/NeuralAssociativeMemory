...
% Fig 1-3: 11
% Fig 4-6: 6
function [processed_pattern_noise,processed_recall_itr,fraction_unsuccess_count] = read_faulty_results_itr_3D(N_in,K_in,L_in,alpha0,beta0,theta0,gamma_BFO,gamma_BFS,recall_algorithm_option,pattern_neur_noise_range,const_neur_noise_range)
warning off
...
for ll = 1:length(N_in)
    ...
    for kji = 1:length(pattern_neur_noise_range)
        ...
        if (recall_algorithm_option == 0)                
            ...
            if (fid > -1)                    
                results_itr = fscanf(fid, '%s %d %s %f %s %f',[6,inf]);
                fclose(fid);                
            else                
            ...
            for i = 1:length(unprocessed_error_bits)
                if (unprocessed_error_bits(i) == err_bits)    
                    ...
                    if (unprocessed_recall_itr(i)>=1000)
                        ...
                        processed_recall_itr = processed_recall_itr + 40;
                    else
...
%             processed_recall_itr = processed_recall_itr - 1000*processed_unsuccess_count;            
        ...
        BER_processed(ihi,kji) = processed_recall_itr;        
        if (isnan(processed_recall_itr))
            111
        end
        %----------------------------------------------------------------------                       
